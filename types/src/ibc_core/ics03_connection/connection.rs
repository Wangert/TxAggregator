use std::{fmt::{Display, Error as FmtError, Formatter}, str::FromStr, time::Duration};
...
use crate::{error::TypesError, ibc_core::{ics23_commitment::commitment::CommitmentPrefix, ics24_host::identifier::{ClientId, ConnectionId}}, timestamp::ZERO_DURATION};

use super::{error::ConnectionError, version::Version};
...
impl TryFrom<RawIdentifiedConnection> for IdentifiedConnectionEnd {
    ...
    fn try_from(value: RawIdentifiedConnection) -> Result<Self, Self::Error> {
        ...
        Ok(IdentifiedConnectionEnd {
            connection_id: value.id.parse().map_err(ConnectionError::invalid_identifier)?,
...
impl TryFrom<RawConnectionEnd> for ConnectionEnd {
    ...
    fn try_from(value: RawConnectionEnd) -> Result<Self, Self::Error> {
        ...
        if value.client_id.is_empty() {
            return Err(ConnectionError::empty_proto_connection_end());
        }

        Ok(Self::new(
            state,
            value.client_id.parse().map_err(ConnectionError::invalid_identifier)?,
            value
                .counterparty
    ...
    fn try_from(value: RawCounterparty) -> Result<Self, Self::Error> {
        ...
        Ok(Counterparty::new(
            ...
            value
                ...
                .key_prefix
                .try_into()
                .map_err(ConnectionError::bad_prefix)?,
        ))
    }
}
...
impl TryFrom<i32> for State {
    ...
    fn try_from(value: i32) -> Result<Self, Self::Error> {
        match value {
            ...
            2 => Ok(Self::TryOpen),
            3 => Ok(Self::Open),
            _ => Err(ConnectionError::invalid_state(value)),
        }
    }
}
