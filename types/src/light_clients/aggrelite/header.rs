...
use utils::pretty::{PrettySignedHeader, PrettyValidatorSet};

use crate::error::TypesError;
use crate::ibc_core::ics02_client::height::Height;
use crate::{ibc_core::ics24_host::identifier::{ChainId, ClientType}, timestamp::Timestamp};

pub const AGGRELITE_HEADER_TYPE_URL: &str = "/ibc.lightclients.aggrelite.v1.Header";
...
impl crate::ibc_core::ics02_client::header::Header for Header {
...
impl TryFrom<RawHeader> for Header {
    ...
    fn try_from(raw: RawHeader) -> Result<Self, Self::Error> {
        let header = Self {
            signed_header: raw
                .signed_header
                .ok_or_else(TypesError::missing_signed_header)?
                .try_into()
                .map_err(|e| TypesError::invalid_header("signed header conversion".to_string(), e))?,
            validator_set: raw
                .validator_set
                .ok_or_else(TypesError::missing_validator_set)?
                .try_into()
                .map_err(TypesError::invalid_raw_header_set)?,
            trusted_height: raw
                .trusted_height
                .and_then(|raw_height| raw_height.try_into().ok())
                .ok_or_else(TypesError::missing_trusted_height)?,
            trusted_validator_set: raw
                .trusted_validators
                .ok_or_else(TypesError::missing_trusted_validator_set)?
                .try_into()
                .map_err(TypesError::invalid_raw_header_set)?,
        };

        if header.height().revision_number() != header.trusted_height.revision_number() {
            return Err(TypesError::mismatched_revisions(
...
impl TryFrom<Any> for Header {
    ...
    fn try_from(raw: Any) -> Result<Self, TypesError> {
        ...
        fn decode_header<B: Buf>(buf: B) -> Result<Header, TypesError> {
            RawHeader::decode(buf).map_err(TypesError::protobuf_decode)?.try_into()
        }
...
pub fn decode_header<B: Buf>(buf: B) -> Result<Header, TypesError> {
    RawHeader::decode(buf).map_err(TypesError::protobuf_decode)?.try_into()
}
