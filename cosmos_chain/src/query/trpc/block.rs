...
pub mod trpc_block_tests {
    ...
    use super::{block_results, latest_block, latest_block_results, test_detail_block_header};
    ...
    pub fn trpc_block_results_works() {
        ...
        let block_results = rt.block_on(block_results(&mut trpc_client, 150 as u32));
    ...
    #[test]
    pub fn trpc_latest_block_results_works() {
        init();
        let file_path =
            "/Users/wangert/rust_projects/TxAggregator/cosmos_chain/src/config/srxc_chain_1.toml";
        let cosmos_chain = CosmosChain::new(file_path);
        ...
        let mut trpc_client = cosmos_chain.tendermint_rpc_client();
        let block_results = rt.block_on(latest_block(&mut trpc_client));
